...
export default function EdupromptPage() {
  ...
  useEffect(() => {
    ;(async () => {
      try {
        const res = await fetch('/api/tools/access?key=eduprompt', { cache: 'no-store' })
  ...
  return (
    <main className="mx-auto max-w-6xl px-4 py-10">
      <h1 className="text-2xl font-semibold">Eduprompt</h1>
      <p className="mt-2 text-muted-foreground">Halaman Eduprompt akan segera diisi.</p>
    </main>
  )
}
